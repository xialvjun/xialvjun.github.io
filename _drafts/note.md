...
==== 文件hash分享系统 ====
对用户使用的文件夹直接分享很容易误操作，把私有文件分享出去了，这很严重（即最好不要把专门用于分享的文件夹与用户使用的文件夹混到一起，甚至都不要有在分享系统里用类似文件浏览器的形式界面，直接选择文件做分享，而是用类似上传的形式界面去做分享这样才更安全）。
所以应该事在各个分区都有一个 store，文件和文件夹都是下载到那里，并且硬链接到用户要使用的位置，采用 copy_on_write 的方式。
但是 NTFS 只有 hardlink 没有 reflink(copy_on_write_link) ，所以是 先创建文件夹，后硬链接文件，然后检测文件变化事件，或变化时间，以及非安全hash，来提示分享完整性。或者用户强制 store 内完整的文件拷贝，保证文件完整性
https://superuser.com/a/1761961/1409656
...
- 表单的布局也有两种方式
  1. 所有字段同样的宽度
  > 简单的可以用百分比, 例如单个字段就是 100%, 这就是常见的小型弹出窗里, 从上到下输入字段的; 复杂的更好的就用 `grid-template-columns: repeat(auto-fit/fill, minmax(300px,1fr))`, 建议用`auto-fit`, [两者区别](https://css-tricks.com/auto-sizing-columns-css-grid-auto-fill-vs-auto-fit/);）
  2. 各字段长度不固定, 使用自己的长度: 就是纯粹的 inline
上面两种维度, 则有 2*2=4 种情况:
同一行相同宽度: label 也应该有相同宽度, 小型编辑表单
同一行不同宽度: label 也是纯粹的 inline, 使用不固定宽度, 适合查询表单
不同行相同宽度: 比较适宜于超多字段(几十个)的大表单, 大型编辑表单
不同行不同宽度: 没有特别要求

还有一个维度是：一行只有一个字段，和一行有多个字段。一行只一个字段，如果字段名与输入框同行，则字段名宽度也相同，且靠右。一行如果有多个字段，看情况设计。


# politic things
用“穷/富”的逻辑来思考问题，永远都达不到共同富裕。应该用“我们想要什么样的生活，需要做哪些事情，哪些是有意义的工作，哪些是狗屎工作”。


# 管理系统权限怎么设置？
有公司，公司下面有车队，车队有车队长，车队长安排员工的排班。车队长进“排班页面”，看到的是自己车队的排班。然后公司老总进“排班页面”，显示什么？然后这个系统给多个公司用，公司之间互相看不到别人的数据，即该系统增加了租户隔离，然后系统开发者进“排班页面”，看到什么？

上面的问题出现的原因，直接原因是：角色可以被自定义。
用角色去决定界面功能（角色），用数据关联去决定能查看哪些数据（租户）。本来这的确不错，但在角色分配不当的时候，就会出现这种问题。

另外还有个思路去思考这个问题，或许这也是其根本原因，就是：把用户与数据混淆了。对于开发者而言，公司，车队长，车队，都是数据。对于公司老总而言，车队长，车队，都是数据。对于车队长而言，车队员工是数据。

假如用 To C 的思路去想，或许角色在一开始程序写出来的时候就定下了。一个美团，有商户端，有用户端，有外卖员端，这就是角色，在程序写出来的时候就定好了。

那么，一方面几乎所有管理系统都希望角色能灵活定义，一方面角色实际上并不能真的灵活定义。

所以，正确的思路应该是：**先有程序角色；后有自定义角色；账户不能跨程序角色；账户没必要跨自定义角色（需要跨越自定义角色的话，可以新建个自定义角色，有两个自定义角色的功能并集）；自定义角色不能跨程序角色（其实就是进一步账户不能跨越程序角色）**。

应用到上面的问题就是：有程序角色（开发者端、公司老总端、车队长端）；有自定义角色（公司老总端里有权限管理，里面有车的管理，车队管理，人的管理，他按自己喜好，安排人去管这些）；公司老总端，更准确的称呼应该叫公司总部端，一个账号不能既是公司总部的，又是某个具体车队的，即账号不能既是公司总部端，又是车队长端；自定义角色不能跨程序角色，公司老总安排人去管车管人，那个人不能既管某车队的司机排班，又管公司下面所有车队的安排，除非那个人有两个账号，或者公司总部端的程序有实现总部的人切换车队，去管具体某个车队的排班的功能。
...
# 基于 excel 的管理系统
先看上面的 **业务未定的管理系统**，那这样一个管理系统应该怎么做呢？
操作人员可以直接点击“创建业务”按钮，然后选择支持 **查询导出、批量导入、单个导入、手动创建、手动修改、删除、批量覆盖、批量删除** 等功能。

- 查询导出：就是查询导出
- 批量导入：选择个 excel 模板（第一行字段中文名，第二行用`{xxx}`语法定义英文名，其实也可以就一行，不需要英文名），这个也定义了有哪些字段。一般几乎所有的业务都支持批量导入，如果此业务不支持批量导入，也可有别的方式定义字段。字段无类型，统一字符串，运行时根据查询逻辑判断是字符串还是数字
- 单个导入：现实中对应 给每个人发一张表格，要求填。选择 excel 模板，遍历所有单元格，按 `{xxx}` 语法找出字段，并灵活判断中文名，xxx 本身是中文，那就是中文名，否则如果左侧有中文单元格，再否则上方有中文单元格，最后就是没有中文名，xxx 就是这个字段的业务名
- 手动创建、手动修改：其实都是普通后台系统的 弹出窗修改，这里只是定义字段而已，甚至都不关心字段 校验逻辑，不关心英文名，不关心唯一索引
- 删除：就是删除
- 批量覆盖：同批量导入，但是要选择 group by 哪几个字段
- 批量删除：仅选择是否支持此功能，在后续的查询中，查出了东西，尽管客户端没有完全加载，做不到全选，也可以点击批量删除，删除当前查询条件能查到的所有条目。批量删除里需要判断此项业务数据在这段时间内有没有变化（需要弹窗确认，或让用户再查一遍，然后自己翻页验证下），需要判断当前查询条件有没有比较模糊的地方（例如在查询中把字符串空当成数字0，需要弹窗显示有哪些数据可能会被意外删除，需要用户验证确认）
- 批量删除+批量导入：可以实现一些复杂点的，不能用 批量覆盖 达成的逻辑

然后是灵活的查询的定义：简单的文本框（等于，空为无条件），文本框完全等于（空就是空），数字框（大于...），下拉框严格等于，树状下拉框（选到底严格等于、不选到底）
- 操作步骤就是：创建查询字段，选择操作符（完全等于就是可搜索的下拉框，如果是大于啥的，就把后面选的业务字段当成数字，有时间处于某范围，有字段严格 in 粘贴从 excel 复制的列，等等），选择被查询的业务字段（如果是完全等于，则可以选择了多个，此时就是树状关联可搜索下拉框）。当然，也就简单模式，直接选择业务字段，默认创建为 文本框模糊搜索。

还有展示字段：
展示，除了展示自己的字段，还可以展示关联字段（要定义关联信息，其实就是对方业务表的查询字段对应这条记录的字段）。因为都是无唯一性约束，所以一关联可能就关联多个，所以表格里展示也是0个1个多个，可以展示关联到的对象的多个字段（用 多行表头-A,B,C[C1,C2] 这种）），选择性展示关联的前几个，也可以展示一个按钮（按钮上有数字表示关联个数），只一个，点击直接弹出跟手动修改一样的只读弹窗，有多个，则展示该业务的查询表格（仅表格部分，无查询部分)


要关联编辑，必须得手动分两步进行。

然后是关联编辑：
两个非稳定业务之间，尽管展示字段里可以有关联，但没有关联编辑的概念，（所有的关联都是假的，不然难道这边关联的字段变了，那边也要变不成。而且 **所谓关联查询，就是对方业务表的查询字段对应这条记录的字段**，假如某个查询是：我是 A，查询 B.c > A.c 的，这种有如何关联改），要说非要做以严格相等为逻辑的关联编辑也可以，但是这样复杂度就大幅增加，错误可能会传播。
不过非稳定业务可以关联到稳定业务上
可以做到稳定业务上：非稳定业务上的字段是外键，关联到稳定业务的主键上。
真等到需要关联编辑了，基本业务应该也稳定了，可以移到稳定业务区了，真正写代码处理业务。

另外，也可以看看 https://www.guanexcel.com/


# 浏览器的音视频录制
https://juejin.cn/post/7052289456455286821
https://stackoverflow.com/a/49438119/3127028 - sampleRate 采样率
https://zhuanlan.zhihu.com/p/422380695
流程：
```js
// 1. 获取 stream
...
# 编程语言 模块设计
https://github.com/koka-lang/koka/issues/333#issuecomment-1507899357
...
似乎 use 是完全没必要的，直接 `use A.E` = `const {E} = A`，你要 use 一个东西，必须得你本身能访问那个东西，既然能访问那个东西，自然命名一个常量就好了
然后关于 文件模块的问题，弄成跟 rust 类似。整个项目只有一个初始模块，lib 是 src/lib.rs ，bin 是 src/main.rs
然后在这个初始模块里，可以用 `mod abc;` 来声明存在 abc 这么个模块，优先查找 src/abc.rs ，然后是 src/abc/lib.rs，然后是 `src/abc/[lib.rs] - 默认的 lib.rs 内容是会 pub mod 该目录下所有的 *.rs 和 文件夹`。。。
要记住，只能声明严格的同目录下的模块，不能 `mod abc/def;` 然后下面直接用 def，因为它把 mod 和 const 混用了，还造成封装泄漏。

https://github.com/koka-lang/koka/issues/333#issuecomment-1514319010

这种逻辑甚至可以用来做整个项目（而不是仅仅自己的 crate）的模块概念
```
// Cargo.toml 里定义了 dependency 有  hyper: ^1.2.3, tokio: ^1.0.5 则项目相当于
mod tokio {
  // tokio ^1.0.5 的内容
}
mod hyper {
...
如果想要自定义模块也叫 tokio，可以使 名字遮盖 shadow ，即
const rust_tokio = tokio;
mod tokio {
  // 自定义的 tokio 模块
}
// 同时，从这点看来，似乎
mod abc { pub fn a() {} }
const abc = { fn a(){}; { a } }
// 两者是一回事，而且恰好，能引用上层不就是 闭包作用域 吗。。。 所以 mod pub 其实就是语法糖
// 有一点难以理解的地方是 mod 里能声明各种非运行时的东西，例如 type，而 const abc = { xxx } 表达式却似乎是运行时的，其实 const abc = {xxx} 不是运行期的，它是编译期的
```

然后子模块与父模块不是同一个文件，需要在最顶部加句 const parent_module = super; 来指明外部有模块，于是 language-server-protocol 会看自己的文件名是不是 mod.rs ，如果是 mod.rs 就找上层目录的 mod.rs ，如果不是 mod.rs ，就找同层目录的 mod.rs 
其实不加，直接用 super 也可以，不过 super 关键词的指向会随模块位置变化


# 语言学习
看 https://news.ycombinator.com/item?id=35662260 后的突然来的点子
语言学习，最重要是两点：
1. 沉浸式
2. 关联性
所以可以在 vr（沉浸式） 里，有个虚拟导游带领游玩全国，体验所有场景，而且一个场景里有很多对话，并不一定在一次内全部出现，可能多次出现，每次说的话不完全相同（关联性），而且可以关联到其他话题，然后切换场景。场景可以实拍，来个 伪6dof，全程用 语音操控。
...
# Yggdrasil-Network 解决 IP 地址过短，不安全的问题
https://github.com/yggdrasil-network/yggdrasil-go/issues/1106
使用长域名，然后 内置 DNS 解析，（如果有必要的话，例如应用不是直接用域名通信，而是先找 DNS 要域名的 IP，此时就需要 fake dns，类似 clash 翻墙软件）
然后又因为
https://blog.csdn.net/zhangmingcai/article/details/116719599
每个段域名长度最大限制是63个字符，域名总长度则不能超过253个字符(不包含最后一个.点)，在域名中大小写是没有区分的。
1、可以包含英文字母（a-z，26个）、数字（0-9，10个），以及半角的连接符“-”（即中横线），不能使用空格及特殊字符（如！、$、&、?等）；
2、“-”不能连续出现，不能单独注册，也不能放在开头或结尾；
3、域名长度有限制，最多可以注册63个字符。
4、在域名中，不区分英文字母的大小写。
所以 256 bit 的域名，需要
这是一个普通文件的 sha256 码 。。。 必须用 . 把 64 位断开
http://FF371A1D16E1F0467010868FAFF0E936.D096C30D2B7890496C426B052F570A8E.ygg
这是把 2**256 得到的最大 10 进制数，78 位。用 base10 方便字母数字切换不方便的输入场景
http://11579.20892.37316.19542.35709.85008.68790.78532.69984.66564.05640.39457.58400.79131.29639.936.ygg
在 hosts 文件中，ygg 后缀也可以不要，直接顶级域名，或者纯数字,也不需要用点断开（只要长度小于64，不用 hex/base16，用 base36），它都能解析，例如
http://FF371A1D16E1F0467010868FAFF0E936D096C30D2B7890496C426B052F570A8:8080
0-9+a-z 是 base36, 移除 0o1l 是 base32
256bit 可以展示为 52 位长度的字符串 (2n ** 256n).toString(32)

另外问了 文心一言，得知 非对称加密，最短的公钥都 2048 位，所以 yggdrasil 中，ip 并非公钥，而是公钥的 hash 值。根据 ip 找到对称点后，先问对方要公钥，再判断公钥 hash 是否跟 ip 一样，之后才发真正的消息。yggdrasil 作者担心的只是 ip 太短，导致能选择的 hash 算法太不安全，他并没有担心公私钥加密不安全。
而这里用长域名，能选择的 hash 算法就很安全了，可以用 sha256/blake3

yggdrasil 初次连接不能直接发消息内容，其实 quic 也一样，必须要先获得并验证公钥（yggdrasil 验证公钥是用 IP 去验证，我们这里是用域名验证，https/quic 是用根证书验证），之后发消息就是 0-RTT 了。

关于 yggdrasil node 充当 stun 服务器，让连接后 延迟降低带宽增加 
